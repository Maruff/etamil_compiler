...
|---|---|---|
| **1. Compiler and core language** | 🟢 Substantially complete | Lexer (202 keywords × 3 spellings), parser with positions on every error, bytecode VM, fixed-point decimal throughout, functions, arrays and records, results, modules, a narrow type checker. **Open:** `செயல்` cannot declare parameter or return types; a chained `a > b > c` is refused rather than chained, because chaining needs the middle operand evaluated once and this AST cannot say "once"; 19 of 202 keywords are off-scheme romanized, and correcting one renames a keyword, which breaks programs already using the old spelling; `மறை` is a repeating-key XOR rather than an AEAD. Closed since this line was written: `மற்றும்`/`அல்லது` short-circuit, `இல்லை` can be an operand of `&`, `ஜேசான்_உரை` is implemented, and the LLVM backend no longer computes in `f64` — it calls a runtime, so decimals are exact and all 59 builtins are reachable |
