...
|---|---|---|
| **1. Compiler and core language** | 🟢 Substantially complete | Lexer (203 keywords × 3 spellings), parser with positions on every error, bytecode VM, fixed-point decimal throughout, functions, arrays and records, results, modules, a narrow type checker, `நிலை` bindings that do not change, functions as values, and record shapes (`வடிவம்`) with methods — all four on both backends. `nUlakam/aNi.qmz` has map, filter and fold (`ஒவ்வொன்றுக்கும்`, `வடிகட்டு`, `மடி`); the loops written before them are left as they are. Outside the core: the LLVM backend still refuses I/O statements (section 7), and connections cannot be named and held concurrently (section 5). Closed since this line was written: `மற்றும்`/`அல்லது` short-circuit; `இல்லை` can be an operand of `&`; `ஜேசான்_உரை` is implemented; the LLVM backend no longer computes in `f64` — it calls a runtime, so decimals are exact and all 62 builtins are reachable; `செயல்` declares parameter and return types and is held to them; `a > b > c` chains, expanding to `மற்றும்` over neighbouring pairs and refusing a middle operand that cannot be read twice; every one of the 203 keywords romanizes on-scheme, reached without a breaking rename by making the scheme's spelling canonical and keeping the old one; and `மறை` is XChaCha20-Poly1305 over an Argon2id key, so tampering is detected rather than decrypted |
...
| **3. Tooling, a REPL shell, database integration** | 🟢 All three | **Database:** SQLite, PostgreSQL and MySQL, each verified against a live server; one database at a time, and the second is now refused rather than swapped. **Tooling:** VS Code extension with grammar and completions generated from `lexer.rs` and a CI gate against drift, `--check`, prebuilt packages, install scripts. **REPL:** `etamil --repl` — `src/repl.rs`. Variables persist in a VM created once and reused; `செயல்` definitions are kept and recompiled each line, which is why definitions are the only statements replayed — replaying the rest would run a session's side effects once per line. A line ending in an unclosed `{` keeps reading until the braces balance, so a `செயல்` or a `சுற்று` can be typed across several lines. `:vars`, `:help`, `:quit` |
...
Against the paper's scheme the project is **mid-Phase 2, with Phase 3 complete**, and Phase 1 close enough to done that what remains in it are known defects rather than missing pieces. Phase 2 is the one with a piece missing rather than a defect: banking is not started.
...
What is true is narrower and already documented above: 94 of the 203 keywords may
be used as names, and the other 109 may not — using one of those as a field name
