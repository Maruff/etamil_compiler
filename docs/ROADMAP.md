...
**MySQL / MariaDB** is implemented behind `--features mysql` and verified
against a live local MySQL server. The complete check is in
`examples/db_samples/mYcIkul_qaLam.qmz`; `run_examples.sh` skips it unless
`ETAMIL_TEST_MYSQL` is set.
